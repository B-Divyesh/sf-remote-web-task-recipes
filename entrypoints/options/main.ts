...
async function init() {
  ...
  if (license?.valid && Date.now() - license.checkedAt > 86_400_000) {
    void verifyLicense(license.token).then(() => render()).catch(() => announce('License check is offline. Cached appearance remains available.'));
  }
}
